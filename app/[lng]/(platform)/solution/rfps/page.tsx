...
export default function RFPsPage() {
  ...
  const filteredRFPs = rfps.filter(rfp => {
    ...
    const matchesSector = filterSector === 'all' || rfp.sector === filterSector;
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            {lng === 'ar' ? 'إدارة RFPs' : 'RFP Management'}
          </h1>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            aria-label={lng === 'ar' ? 'تصفية حسب القطاع' : 'Filter by sector'}
          >
            <option value="all">{lng === 'ar' ? 'جميع القطاعات' : 'All Sectors'}</option>
                ...
                filteredRFPs.map((rfp) => (
                  <tr key={rfp.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right rtl:text-right">
                      ...
                      {rfp.client_industry && (
                        <div className="text-sm text-gray-500">{rfp.client_industry}</div>
                      )}
                    </td>
