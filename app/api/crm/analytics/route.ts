/**
 * CRM Analytics API
 * Advanced analytics for CRM module
 */
...
export const GET = withRateLimit(async (request: NextRequest) => {
  try {
    ...
    // Build cache key
    const cacheKey = `${CACHE_PREFIXES.ANALYTICS}crm:${tenantId}:${dateFrom || 'all'}:${dateTo || 'all'}`;

    // Get or fetch analytics with caching
    const analytics = await multiLayerCache.getOrFetch(
      cacheKey,
      async () => {
        return await CRMService.getCRMAnalytics(tenantId, {
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to fetch CRM analytics' },
      { status: 500 }
    );
  }
