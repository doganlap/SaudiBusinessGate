...
export async function GET(request: NextRequest) {
  try {
    ...
    const receivables = result.rows.map((row: any) => ({
      id: row.id,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch accounts receivable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const checkQuery = `SELECT id FROM accounts_receivable WHERE tenant_id = $1 AND invoice_number = $2`;
    ...
    const query = `
      INSERT INTO accounts_receivable (
        ...
        category, description, invoice_id, created_by
    ...
    return NextResponse.json(
      { error: 'Failed to create accounts receivable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to update accounts receivable', message: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
