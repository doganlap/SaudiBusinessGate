...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const result = await query('SELECT * FROM financial_accounts WHERE id = $1', [params.id])
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const fields = ['account_name','account_code','account_type','balance','parent_account_id','is_active','description']
  ...
  const sql = `UPDATE financial_accounts SET ${updates.join(', ')}, updated_at = NOW() WHERE id = $${i} RETURNING *`
  const result = await query(sql, values)
  if (result.rowCount === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  await audit.logDataChange(userId, organizationId, 'financial_account', (result.rows[0] as any).id, null, result.rows[0])
  return NextResponse.json(result.rows[0])
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const before = await query('SELECT * FROM financial_accounts WHERE id = $1', [params.id])
  const result = await query('DELETE FROM financial_accounts WHERE id = $1 RETURNING id', [params.id])
  if (result.rowCount === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 })
  await audit.logDataChange(userId, organizationId, 'financial_account', (result.rows[0] as any).id, before.rows[0], null)
