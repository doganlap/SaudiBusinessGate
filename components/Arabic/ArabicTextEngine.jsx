...
const ArabicTextEngine = ({ 
  ...
  const personalityClasses = {
    friendly: 'font-normal tracking-wider leading-relaxed text-gray-700',
    formal: 'font-medium tracking-wide leading-snug text-gray-900',
    casual: 'font-light tracking-widest leading-loose text-gray-600',
    professional: 'font-normal tracking-wider leading-relaxed text-gray-700',
  };

  const combinedClassName = `
    relative inline-block transition-all duration-300 ease-in-out
    ${isRTL ? 'font-amiri text-right' : 'font-english text-left'}
    ${calligraphy ? 'p-5' : ''}
    ${personalityClasses[personalityType] || personalityClasses.professional}
    ${className}
  `.trim();
      ...
      >
        {typewriter ? (
          <span className="inline-block relative">
            {displayText}
            {isTyping && <span className={`inline-block bg-current w-0.5 animate-blink ${isRTL ? 'mr-0.5' : 'ml-0.5'}`}></span>}
          </span>
        ) : (
          children
        )}
        ...
        {voice && (
          <motion.button
            className={`absolute -top-1 bg-transparent border-none text-base cursor-pointer opacity-70 hover:opacity-100 transition-opacity duration-300 ease-in-out z-10 ${isRTL ? 'right-auto -left-8' : '-right-8'}`}
