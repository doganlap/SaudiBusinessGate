...
export class EmailService {
  ...
  async sendSubscriptionUpdate(params: {
    ...
    try {
      ...
      await this.sendEmail({
        to: await this.getTenantContactEmails(params.tenantId),
        subject,
        html: content,
        text: content,
        templateName: 'subscription_update',
        metadata: { tenantId: params.tenantId, subscriptionId: params.subscriptionId }
      });
      return true;
    } catch (error) {
  ...
  async sendUsageOverageAlert(params: {
    ...
    try {
      ...
      await this.sendEmail({
        ...
        html: content,
        text: content,
        templateName: 'usage_overage',
        metadata: { tenantId: params.tenantId, charges: params.charges.total }
      });
      return true;
    } catch (error) {
